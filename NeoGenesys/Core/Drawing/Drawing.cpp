...
namespace NeoGenesys
{
	...
	*/
	void cDrawing::DrawPlayer(sCEntity* entity, ImVec2 bones2d[BONE_MAX], ImVec2 corners2d[8], ImVec2 center, float scale, float distance, std::string name, ImVec4 color)
	{
		...
		bool bHasRiotShield = ((BYTE)entity->NextEntityState.iWeapon == WEAPON_RIOT_SHIELD || (BYTE)entity->NextEntityState.LerpEntityState.iSecondaryWeapon == WEAPON_RIOT_SHIELD);
	...
	*/
	void cDrawing::DrawItem(std::string name, ImVec2 center, float distance, ImVec4 color)
	{
		...
		DrawString(szText, ImVec2(center.x - vStringSize.x / 2.0f, center.y - vStringSize.y / 2.0f), 14.0f, true, _profiler.gColorText->Custom.cValue);
	}
	/*
	//=====================================================================================
	*/
	...
	*/
	void cDrawing::DrawAgent(sCEntity* entity, ImVec2 center, float distance, ImVec4 color)
	{
		std::string szText(VariadicText("[%im] %s", (int)(distance / M_METERS), 
			...
			((BYTE)entity->NextEntityState.iWeapon == WEAPON_RIOT_SHIELD ||
			(BYTE)entity->NextEntityState.LerpEntityState.iSecondaryWeapon == WEAPON_RIOT_SHIELD) ?
		...
		for (int i = 0; i < MAX_ENTITIES; i++)
		{
				...
				if (_targetList.EntityList[i].bW2SSuccess && _targetList.EntityList[i].bIsValid)
				{
						...
						_profiler.gWallHackMode->Custom.iValue == cProfiler::WALLHACK_MODE_ALL)
					{
						DrawPlayer(&CEntity[i], _targetList.EntityList[i].vBones2D, _targetList.EntityList[i].vCorners2D, _targetList.EntityList[i].vCenter2D, _targetList.EntityList[i].vLower.y - _targetList.EntityList[i].vUpper.y,
							GetDistance3D(CEntity[i].vOrigin, CG->PlayerState.vOrigin), ClientInfo[i].szName, _targetList.EntityList[i].cColor);
					}
				}
			} break;
			...
			case ET_ITEM:
			{
				if (_profiler.gItems->Custom.bValue)
					if (_targetList.EntityList[i].bW2SSuccess && _targetList.EntityList[i].bIsValid)
						DrawItem(_targetList.EntityList[i].szWeapon, _targetList.EntityList[i].vCenter2D, GetDistance3D(CEntity[i].vOrigin, CG->PlayerState.vOrigin), _targetList.EntityList[i].cColor);
			} break;
			...
			case ET_MISSILE:
			{
				if (_profiler.gMissiles->Custom.bValue)
					if (_targetList.EntityList[i].bW2SSuccess && _targetList.EntityList[i].bIsValid && _targetList.EntityIsEnemy(i) &&
						CEntity[i].NextEntityState.iOtherEntityNum < FindVariable("sv_maxclients")->Current.iValue)
						DrawMissile(_targetList.EntityList[i].szWeapon, _targetList.EntityList[i].vCenter2D, GetDistance3D(CEntity[i].vOrigin, CG->PlayerState.vOrigin), _targetList.EntityList[i].cColor);
			} break;
			...
			case ET_AGENT:
			{
				if (_profiler.gAgents->Custom.bValue)
					if (_targetList.EntityList[i].bW2SSuccess && _targetList.EntityList[i].bIsValid && _targetList.EntityIsEnemy(i))
						DrawAgent(&CEntity[i], _targetList.EntityList[i].vCenter2D, GetDistance3D(CEntity[i].vOrigin, CG->PlayerState.vOrigin), _targetList.EntityList[i].cColor);
			} break;
			}
		}
	}
